...
import {
    postRemoveUserFromParticipantsAPICALL,
    postAddUserToParticipantsOfEventAPICALL
} from '../../utils/requests';
...
class ViewParticipants extends Component {
  constructor (props) {
    ...
    console.log(this.props.history.location.state.ParticipantsReady);
      // schemaParticipants.properties.listOfStrings.push(this.props.history.location.state.ParticipantIDs[0]);

      this.setState({
          users: this.props.history.location.state.ParticipantsReady
      });
    ...
    handleSearch(text, currentUsers){
        ...
        return postAddUserToParticipantsOfEventAPICALL(this.props.history.location.state.hashedIDEvent, text, currentUsers)
            .then((res) => {
                if(res.data.status === "success"){
                    ...
                    this.props.history.location.state.ParticipantsReady.unshift(res.data.data.userData)
                    console.log(this.props.history.location.state.ParticipantsReady);
                    this.setState({
                        users: this.props.history.location.state.ParticipantsReady
                    });
    ...
    removeUserFromParticipationOfAnEvent(hashedIDEvent, hashedID, auiID){
      console.log("HNA");
      console.log(currentUsers);
        return postRemoveUserFromParticipantsAPICALL(hashedIDEvent, hashedID)
            .then(isDeleted => {
                return isDeleted;
            })
    }
    ...
    createTable = () => {
        let table = [];
        this.state.users = this.props.history.location.state.ParticipantsReady;
                                        ...
                                        <div >
                                            ...
                                            <button  className='btn btn-danger' style={{width: "100%"}} onClick={()=>{
                                                ...
                                                if(answer === participant.auiID){
                                                    this.removeUserFromParticipationOfAnEvent(this.props.history.location.state.hashedIDEvent,participant.hashedID, participant.auiID)
                                                        .then((isDeleted)=>{
                                                            if(isDeleted){
  ...
  render () {
    return (
        <div>
            <div>
                <CSVLink filename={"event_participants.csv"} data={csvData} className="btn btn-primary">Download CSV File</CSVLink>
                <div>
            ...
            <label style={{marginBottom: '50px'}}>Number of participants: {this.props.history.location.state.ParticipantsReady.length}</label>
